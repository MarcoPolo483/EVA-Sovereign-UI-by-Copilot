...
- `eva-gc-language-switcher` (accessible locale menu)
...
Accessible language switcher listing available locales with keyboard navigation.

**Attributes:**
- `include`: comma-separated list of BCP-47 locales to display (e.g., `"en-CA,fr-CA,en-GB"`). Defaults to all available locales from the i18n service.
- `url-mode`: `"prefix" | "none"` (default: `"prefix"`). When `"prefix"`, switching updates the path prefix (e.g., `/en/...`). When `"none"`, emits an event without navigation.

**Events:**
- `locale-change`: fired when a locale is chosen (only for `url-mode="none"`).
  - `detail`: `{ locale: string }`

**Keyboard:**
- Trigger: `Enter/Space` opens; `ArrowDown` opens and focuses first item.
- Menu: `ArrowUp/ArrowDown` moves focus; `Enter/Space` selects; `Escape` closes and returns focus to trigger.

**ARIA:**
- Uses ARIA menu pattern with `role="menu"` and `menuitemradio` for items, labelled by the trigger.

**Examples:**
```html
<!-- Event-only mode (no URL change) -->
<eva-gc-language-switcher include="en-CA,fr-CA,en-US" url-mode="none"></eva-gc-language-switcher>

<!-- URL prefix mode (updates /en or /fr path) -->
...
This project can generate a `custom-elements.json` manifest for IDEs and documentation tools.

- Generate: `npm run custom-elements:generate`
- Output: `custom-elements.json` at repo root
- Publish: To include in the npm package, add `"custom-elements.json"` to `files` in `package.json`.

## Storybook Coverage

Interactive stories are available for design tokens and the language switcher:

- Tokens: `Tokens/Colors`, `Tokens/Typography`, `Tokens/Spacing`, `Tokens/Shadows`
- Language Switcher: `GC Design/i18n/Language Switcher (GC)` with prefix and event modes
