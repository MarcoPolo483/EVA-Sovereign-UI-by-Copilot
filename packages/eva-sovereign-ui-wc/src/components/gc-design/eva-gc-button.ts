...
export class EVAGCButton extends EVABaseComponent {
  ...
  connectedCallback() {
    // Set role for accessibility - allows aria-label on host
  ...
  protected render(): void {
    ...
    // Forward aria-label from host to button for icon-only buttons
    const hostAriaLabel = this.getAttr('aria-label');
    if (hostAriaLabel) {
      button.setAttribute('aria-label', hostAriaLabel);
      // Remove from host to avoid duplication warning
      ...
      if (size === 'icon' && !hasLightText) {
        button.setAttribute('aria-label', this.t('common.button'));
      }
    }
