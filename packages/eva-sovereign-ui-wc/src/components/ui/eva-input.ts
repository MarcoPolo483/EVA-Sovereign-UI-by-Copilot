...
export class EVAInput extends EVABaseComponent {
  ...
  static get observedAttributes() {
    return ['type', 'value', 'placeholder', 'disabled', 'readonly', 'label', 'error', 'aria-label'];
  }
  ...
  attributeChangedCallback() {
    if (this.inputEl) {
      ...
      // Accessible name via label or aria-label fallback
      if (label && this.labelId) {
        ...
        if (ariaLabel) {
          ...
          // Default accessible name when no label provided
          this.inputEl.setAttribute('aria-label', 'Input');
        }
      }
